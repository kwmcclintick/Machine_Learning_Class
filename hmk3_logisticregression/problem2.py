...
'''
    ...
    The main goal of this problem is to get familiar with a model-based classification method, and how to train the
     model parameters on the training data.
...
def compute_a(z):
    ...
    '''
    #########################################
    # I read online that math.exp() can only handle abs(z) < 700
    if z > 700:
        z = 700
    if z < -700:
        z = -700
...
def train(X, Y, alpha=0.001, n_epoch=100):
    '''
       Given a training dataset, train the logistic regression model by iteratively updating the weights w and bias b
       using the gradients computed over each data instance.
We repeat n_epoch passes over all the training instances.
        Input:
            X: the feature matrix of training instances, a float numpy matrix of shape (n by p). Here n is the number
            of data instance in the training set, p is the number of features/dimensions.
