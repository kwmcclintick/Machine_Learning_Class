...
def compute_a(z):
    ...
    '''
    #########################################
    a = np.matrix(np.zeros(shape=z.shape))
    z_temp = z - np.amax(z)
    z_no_overflow = z_temp[z_temp > -700]

    for i in range(len(z)):
        if z_temp[i] < -700:
            a[i] = 0
        else:
            a[i] = np.exp(z_temp[i]) / np.sum(np.exp(z_no_overflow))
    #########################################
    return a
...
def compute_dL_da(a, y):
    ...
    '''
    #########################################
    dL_da = np.mat(np.zeros(shape=(a.shape[0],1)))
...
def compute_da_dz(a):
    ...
    #########################################
    c = a.shape[0]
    da_dz = np.mat(np.ones(shape=(c,c)))
...
def compute_dz_dW(x,c):
    ...
    #########################################
    p = x.shape[0]
    dz_dW = np.mat(np.ones(shape=(c,p)))
...
def compute_dz_db(c):
    ...
    '''
    #########################################
    dz_db = np.mat(np.ones(shape=(c,1)))
    #########################################
    return dz_db
...
def compute_dL_dz(dL_da,da_dz):
    ...
    '''
    #########################################
    dL_dz = np.matmul(dL_da.T, da_dz).T
    #########################################
    return dL_dz
...
def predict(Xtest, W, b):
    ...
    for i, x in enumerate(Xtest):
        x = x.T # convert to column vector
        #########################################
        P[i] = compute_a(compute_z(x, W, b)).T
        Y[i] = np.argmax(P[i])
        #########################################
    print(Y)
    return Y, P 
