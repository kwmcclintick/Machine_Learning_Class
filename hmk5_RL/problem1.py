    ...
    def __init__(self, n, e=0.1):
        ''' Initialize the agent. 
            ...
            Outputs:
                ...
                self.e: the probability of the agent randomly pulling a lever with uniform probability. e is a
                float scalar between 0. and 1.
                self.Q: the expected ratio of rewards for pulling each lever, a numpy vector of length n.
                We initialize the vector with all-zeros.
                self.c: the counts of the number of times that each lever being pulled. a numpy vector of length n,
                initialized a all-zeros.
                
        '''
        #########################################
        self.n = n
        self.e = e
        self.Q = np.zeros(shape=n)
        self.c = np.zeros(shape=n)
        #########################################
   ...
   # ----------------------------------------------
    def forward(self):
        '''
            ...
            The agent will 
                (1) with a small probability (epsilon or e), randomly pull a lever with uniform distribution on all
                levers (Exploration);
                (2) with a big probability (1-e) to pull the arm with the largest expected reward (Exploitation). If
                there is a tie, pick the one with the smallest index.
            Output:
