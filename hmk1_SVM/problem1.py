...
def compute_Phi(x,p):
    ...
    '''
    #########################################
    Phi = np.matrix(np.empty(shape=(p, len(x))))
    for power in range(p):
        Phi[power] = np.reshape(np.power(x, power), (len(x),))
...
def ridge_regression(Phi, y, alpha=0.001):
    ...
    '''
    #########################################
    w = np.linalg.inv(np.transpose(Phi)*Phi + alpha*np.eye(min(np.shape(Phi))))*np.transpose(Phi)*y
    #########################################
    return w 
