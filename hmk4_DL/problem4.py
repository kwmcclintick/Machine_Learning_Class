...
class LSTM(sr):
    ...
    def gates(self, x, H):
        ...
        sig = nn.Sigmoid()
        # forget gate
        f = sig(th.mm(th.cat((x, H), 1), self.W_f) + self.b_f)
        # input gate, ~C
        i = sig(th.mm(th.cat((x, H), 1), self.W_i) + self.b_i)
        # output cell
        o = sig(th.mm(th.cat((x, H), 1), self.W_o) + self.b_o)
        # candidate cell
